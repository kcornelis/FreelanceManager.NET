...
namespace FreelanceManager.ReadModel.TimeRegistrationTests
{
    public class when_a_timeregistration_rate_is_refreshed : Specification
    {
        ...
        {
            _timeregistrationHandler.AsDynamic().Handle(new TimeRegistrationRateRefreshed(_timeregistrationId, 70M));
        ...
        {
            _timeregistration.Rate.Should().Be(70M);
        }

        [Fact]
        public void should_have_a_new_income()
        {
            _timeregistration.Income.Should().Be(140M);
        }
    }
}
